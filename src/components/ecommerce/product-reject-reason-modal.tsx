    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 py-2">
          ...
          {selectedReason === "Other" && (
            <div>
              ...
              <textarea className="w-full border rounded-md px-2 py-1" rows={3} value={customReason} onChange={(e) => setCustomReason(e.target.value)} />
            </div>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          <Button className="bg-destructive text-destructive-foreground" onClick={handleConfirm} disabled={selectedReason === "Other" && customReason.trim() === ""}>
            Reject
          </Button>
        </DialogFooter>
      </DialogContent>
