...
export default function RegisterModal({ open, onOpenChange, onSave, availableDevices, initialData }: RegisterModalProps) {
  ...
  const deviceId = `MASH-${formData.model || "---"}-${locationYear || "---"}-${uniqueDecimal}`
  ...
  const validateForm = () => {
    ...
    if (!selectedDeviceId && !formData.model.trim()) newErrors.model = "Model is required"
                ...
                <div className="space-y-2">
                  ...
                  <div className="mt-2">
                    ...
                    <div className="px-3 py-2 bg-primary/10 rounded-md text-foreground font-mono border border-primary/20">
                      {selectedDeviceId ? (availableDevices.find((a) => a.id === selectedDeviceId)?.deviceId ?? "-") : (availableDevices.length > 0 ? "Select a device" : "No devices available")}
                    </div>
                  </div>
                </div>
              ) : (
