...
export default function UsersManagement() {
  ...
  const fetchUsers = useDashboardStore((s) => s.fetchUsers);
  const storeUsers = useDashboardStore((s) => s.users);
  ...
  const filteredUsers = useMemo(() => {
    ...
    const source = storeUsers ?? MOCK_USERS;
    ...
    return source.filter((user) => {
      ...
      const matchesRole =
        selectedRolesMulti.length > 0
          ? selectedRolesMulti.includes(String(user.role ?? ""))
          : true;

      let matchesStatus = true;
      if (selectedStatuses.length > 0) {
        matchesStatus = selectedStatuses.includes(String(user.status ?? ""));
      } else {
        if (statusFilter === "All") {
                      ...
                      <label className="flex items-center gap-2 px-2 py-1 rounded hover:bg-accent">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setSelectedRegions((prev) =>
                              val
                                ? Array.from(new Set([...prev, "Quezon City"]))
                      ...
                      <label className="flex items-center gap-2 px-2 py-1 rounded hover:bg-accent">
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.checked;
                            setCurrentPage(1);
                            setSelectedRolesMulti((prev) =>
                              val
                                ? Array.from(new Set([...prev, "Seller"]))
                      ...
                      <label className="flex items-center gap-2 px-2 py-1 rounded hover:bg-accent">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setSelectedRolesMulti((prev) =>
                              val
                                ? Array.from(new Set([...prev, "Customer"]))
