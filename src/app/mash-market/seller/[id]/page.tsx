...
export default function SellerDetailPage({ params }: { params: { id: string } }) {
      ...
      <div className="min-h-screen bg-background p-6">
        <div className="mx-auto max-w-4xl">
          <Card className="p-6">
            ...
            <p className="text-sm text-muted-foreground mt-2">We couldn't find a seller with that id.</p>
            <div className="mt-4">
              <Button onClick={() => router.push("/mash-market/seller")}>Back to sellers</Button>
            </div>
          </Card>
        </div>
      </div>
  ...
  const handleReject = async (reason?: string) => {
    ...
    try {
      ...
      list = list.map((s: any) => (s.id === seller.id ? { ...s, status: "rejected", rejectReason: reason } : s))
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-4xl">
        <div className="flex items-center justify-between mb-4">
          ...
          <div>
            <Button variant="ghost" onClick={() => router.push("/mash-market/seller")}>Back</Button>
          </div>
        </div>
        ...
        <Card className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input value={seller.businessName ?? ""} disabled readOnly className="mt-1" />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground">Business Type</label>
                <Input value={seller.businessType ?? ""} disabled readOnly className="mt-1" />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground">TAX ID Number</label>
                <Input value={seller.taxIdNumber ?? ""} disabled readOnly className="mt-1" />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <Input value={(seller.typesOfMushroom || []).join(", ")}
                disabled readOnly className="mt-1" />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground">Monthly production capacity</label>
                <Input value={seller.monthlyProductionCapacity ?? ""} disabled readOnly className="mt-1" />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground">Certifications</label>
                <Input value={(seller.certifications || []).join(", ")}
                  disabled readOnly className="mt-1" />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input value={seller.bankName ?? ""} disabled readOnly className="mt-1" />
              </div>
              <div>
                <label className="block text-sm font-medium text-muted-foreground">Account number</label>
                <Input value={seller.accountNumber ?? ""} disabled readOnly className="mt-1" />
              </div>
              <div className="col-span-2">
                <label className="block text-sm font-medium text-muted-foreground">Account holder name</label>
                <Input value={seller.accountHolderName ?? ""} disabled readOnly className="mt-1" />
              </div>
            </div>
          </div>
          ...
          {seller.status === "pending" && (
            <div className="mt-6 flex gap-3">
              <Button onClick={() => setRejectModalOpen(true)} variant="destructive" disabled={loading}>
          ...
          <RejectReasonModal open={rejectModalOpen} onOpenChange={setRejectModalOpen} onConfirm={(reason) => handleReject(reason)} />
        </Card>
      </div>
    </div>
  )
