...
export default function PendingProductDetail({ params }: { params: { id: string } }) {
  ...
  const handleReject = async (reason?: string) => {
    ...
    try {
      ...
      list = list.map((p: any) => (p.id === product.id ? { ...p, status: "rejected", rejectReason: reason } : p))
        ...
        <Card className="p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input value={formatPricePHP(product.price ?? 0)} readOnly className="mt-1" />
            </div>
            <div>
              <label className="text-sm font-semibold text-muted-foreground">Date Submitted</label>
              <Input value={product.submittedAt ? formatDate(product.submittedAt) : ""} readOnly className="mt-1" />
            </div>
            <div>
              <label className="text-sm font-semibold text-muted-foreground">Status</label>
              <div className="mt-2">{product.status ? <StatusBadge status={product.status as any} /> : "-"}</div>
            </div>
          </div>
          ...
          <div className="mt-6 flex gap-3">
            <Button variant="destructive" onClick={() => setRejectModalOpen(true)} disabled={loading}>
          ...
          <ProductRejectReasonModal open={rejectModalOpen} onOpenChange={setRejectModalOpen} onConfirm={(reason) => handleReject(reason)} />
        </Card>
      </div>
    </div>
  )
